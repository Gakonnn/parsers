...
def parse_arguments() -> tuple[argparse.Namespace, Configuration]:
    ...
    browser_parser.add_argument('--chrome.proxy-server', metavar='URL', help='Прокси-сервер для исходящего трафика Chrome, например socks5://user:pass@host:port')
    browser_parser.add_argument('--chrome.proxy-bypass-list', metavar='LIST', help='Список исключений для прокси Chrome, например localhost;127.0.0.1')
    ...
    rest_parser.add_argument('--web-host', metavar='HOST', default='127.0.0.1', help='Хост веб-интерфейса')
    rest_parser.add_argument('--web-port', metavar='PORT', type=int, default=8080, help='Порт веб-интерфейса')
    rest_parser.add_argument('--web-open-browser', metavar='{yes,no}', default='yes', choices=('yes', 'no'), help='Открыть интерфейс в браузере автоматически')
    ...
    if not GUI_ENABLED and not args.web:
        required_args = {
            '--url': args.url,
        ...
        if missing_args:
            arg_parser.error(f'следующие аргументы обязательны: {", ".join(missing_args)}')
...
def main() -> None:
    ...
    if args.web:
        app(args.url, args.output_path, args.format, config,
            host=args.web_host,
            port=args.web_port,
            open_browser=args.web_open_browser == 'yes')
    else:
        app(args.url, args.output_path, args.format, config)
